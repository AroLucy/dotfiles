    ...
    parseHook = function() {
        ...
        if(result.invalidEmojis.length !== 0) {
            for(let emoji of result.invalidEmojis) {
                result.content = result.content.replace(`<${emoji.animated ? "a" : ""}:${emoji.originalName || emoji.name}:${emoji.id}>`, emoji.url.split("?")[0] + "?size=32");
            }
...
return function() { return {
    ...
    getDescription: () => "Link emojis if you don't have nitro! Type them out or use the emoji picker! [32px]",
    getVersion: () => "1.3",
    getAuthor: () => "An0",
